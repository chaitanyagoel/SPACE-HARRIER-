...
function draw() {
  ...
  if(GAMESTATE === play_INSTRUCTION) {
    ...
    fill("white")
    text("THANK YOU FOR USING SPACE HARRIER, TO PLAY THIS GAME YOU NEED A PC..",width/9.5,height-height/1.5);
    text(", IF YOU WANT TO PLAY THIS GAME ,TO MOVE YOUR SPACESHIP, USE YOUR ARROW KEYS. ",width/10.5,height-height/1.7);
    text(" YOUR MAIN MOTO IS TO DESTROY ALL THE SPACESHIP,HOW MUCH YOU CAN,",width/10.5,height-height/1.9)
    text("  REMEMBER BE SAFE FROM ASTERIODS. AND DONT FORGET THAT ENEMY,",width/10.5,height-height/2.1)
    text("SPACESHIP CAN ALSO SHOOT YOU. YOU CAN ALSO SHOOT BY PRESSING ENTER...",width/10.5,height-height/2.4);
    text("TO HELP YOU IN THIS MISSION WE HAVE ROBOT FRIEND FOR YOU",width/10.5,height-height/2.7)
    text("-THANK YOU ",width/10.5,height-height/3.0);
...
drawSprites();
      ...
      if(GAMESTATE === PLAY) {
        ...
        if(player_Friend_Life >= 1) {
             ...
             textSize(30);
             textFont('Italic')
             text("ROBOTS LIFE: "+player_Friend_Life,400,40);
             text("ROBOT ",computerplayer.x-60,computerplayer.y-50)
        } else{
          fill("white")
          textSize(30);
          textFont('Italic');
          text("ROBOT DIED",400,40)
        }
        fill("white")
        textSize(30);
        textStyle(ITALIC);
 ...
 function spawnSpaceShip() {
    if(frameCount % 22 === 0) {
      ...
      enemy.lifetime = 350;
     // enemy.debug=true;
      enemy.setCollider("rectangle",0,0,10,40);
    ...
    function spawnPlayerBullet() {
      var playerbullet = createSprite(player.x+25,player.y,10,10);
    ...
    function spawnSpaceShip1() {
      ...
      function spawnCompBULLET() {
        if(frameCount % 10 === 0) {
          playershootingmp3.play();
          var bullet = createSprite(computerplayer.x+25,computerplayer.y,20,20);
